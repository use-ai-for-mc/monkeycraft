...
public class MapDataHandler {
  ...
  private byte getEntityType(Entity entity) {
    ...
    // Also include any entity that has passengers or is a vehicle
  ...
  private byte[] buildFrame(
      ...
      List<EntityInfo> entities) {
    // Format:
    // 2 bytes: magic (0x4D 0x4D)
    // 8 bytes: playerX (double)
    // 8 bytes: playerZ (double)
    // 4 bytes: playerYaw (float)
    // 2 bytes: uuidLength (short)
    // N bytes: playerUuid (UTF-8)
    // 2 bytes: entityCount (short)
    // For each entity:
    //   1 byte: entityType
    //   8 bytes: entityX (double)
    //   8 bytes: entityZ (double)
    //   4 bytes: entityId (int)
    //   2 bytes: nameLength (short)
    //   N bytes: name (UTF-8)
