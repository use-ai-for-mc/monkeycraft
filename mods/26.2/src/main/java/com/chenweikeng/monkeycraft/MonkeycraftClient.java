  ...
  private void registerTickEvents() {
    ClientTickEvents.END_CLIENT_TICK.register(
        client -> {
          ...
          WebSocketServerHandler.getInstance().updateScreenState(client.gui.screen());

          if (WebSocketServerHandler.getInstance().isStreaming()
              && client.gui.screen() != null
              && !ScreenHelper.shouldKeepScreen(client.gui.screen())
              && !hasRecentLocalKeyInput()) {
            if (client.gui.screen() instanceof net.minecraft.client.gui.screens.ChatScreen) {
              client.gui.hud.getChat().restoreChatScreen();
            }
  ...
  private void registerCommands(
    ...
    dispatcher.register(
        ClientCommands.literal("monkey")
            ...
            .then(
                ClientCommands.literal("tailscale")
                    .then(
