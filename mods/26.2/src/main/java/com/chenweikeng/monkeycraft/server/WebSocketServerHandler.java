...
public class WebSocketServerHandler {
  ...
  public boolean startServer(int port, boolean isAutoLaunch, boolean persistent) {
    ...
    synchronized (lifecycleLock) {
      if (shutdownRequested.get() || lifecycleState != LifecycleState.STOPPED) {
        return false;
      }
    ...
    try {
      ...
      ShutdownSequence.run(
          ...
          new ShutdownSequence.Step(
              "H264 streamer",
              () -> {
                if (streamerToStop != null) {
                  streamerToStop.close();
                }
              }),
          new ShutdownSequence.Step(
              "input state",
  ...
  public void setConnectedDeviceName(String name) {
    connectedDeviceName = name == null ? "" : name;
  }
  ...
  public void beginPairing(WebSocket conn, String deviceName) {
    ...
    PairingSession session = new PairingSession(conn, deviceName);
    ...
    String who =
        session.deviceName().isEmpty() ? "Phone at " + ip : session.deviceName() + " (" + ip + ")";
            ...
            MonkeycraftClient.sendMonkeyMessage(
                Component.literal(
                    who
                        ...
                        + "). Allow in the prompt, or /monkey accept "
                        + session.code()));
            MonkeycraftClient.openPairingPrompt(session.code(), ip, session.deviceName());
          });
    }
  }

  public boolean rejectPairing() {
    return rejectCurrentPairing();
  }

  public boolean rejectPairing(String typedCode) {
    PairingSession session = pairingSession.get();
    if (session == null || typedCode == null || !session.matches(typedCode)) {
      return false;
    }
    return clearPairing(session);
  }

  private boolean rejectCurrentPairing() {
    PairingSession session = pairingSession.get();
    if (session == null) {
      return false;
    }
  ...
  private boolean clearPairing(PairingSession session) {
    ...
    if (session.conn().isOpen()) {
      JsonObject failed = new JsonObject();
      failed.addProperty("type", "PAIR_FAILED");
      failed.addProperty("message", "Pairing declined on the computer");
      session.conn().send(GSON.toJson(failed));
      session.conn().close();
    }
    return true;
  }
  ...
  private boolean isIpAddressAllowed(InetAddress addr) {
    return com.chenweikeng.monkeycraft.utils.NetworkUtils.isConnectionAllowed(
        ModConfig.getInstance().getNetworkScope(), addr);
  }
  ...
  private class MonkeycraftWebSocketServer extends WebSocketServer {
    ...
    public void onOpen(WebSocket conn, ClientHandshake handshake) {
      ...
      hello.addProperty("keyId", ModConfig.getInstance().getPasswordId());
    ...
    private void handleClientStatus(WebSocket conn, JsonObject json) {
      ...
      if (clientMode == ClientMode.STREAMING && json.has("width") && json.has("height")) {
        ...
        if (!configureStreamer(targetWidth, targetHeight, colorMode, fps, dataSaver)) {
          return;
        }
