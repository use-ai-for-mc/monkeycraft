...
echo "building GOOS=$GOOS GOARCH=$GOARCH helperVersion=$VERSION tailscale=$TS_VERSION commit=$COMMIT"
CGO_ENABLED=0 GOOS="$GOOS" GOARCH="$GOARCH" go build -trimpath -ldflags "$LDFLAGS" -o "$OUT_DIR/$NAME" ./cmd/monkeycraft-tailscale-helper
