...
### 2026-09-24 Pages 发布后浏览器旧缓存修复

c3980a4的Pages运行35946147702构建及部署成功；线上来源记录dirty=false，12项主入口和Tailscale资源哈希均匹配。实际Chrome以及用户手机却仍显示旧地址表单。Chrome读回没有Service Worker控制，main.dart.js仍为旧3007143字节，普通刷新也复现；线上新程序为3049348字节。问题在浏览器HTTP缓存复用固定脚本URL，而非Tailscale登录状态或构建未部署。
