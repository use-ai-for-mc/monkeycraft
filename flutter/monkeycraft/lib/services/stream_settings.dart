...
class StreamSettingsStore {
  ...
  Future<StreamSettings> load() async {
    ...
    final colorMode = prefs.getInt(_kColorMode) ?? StreamSettings.defaults.colorMode;
    final presetIndex =
        prefs.getInt(_kResolutionPreset) ?? StreamSettings.defaults.resolutionPreset.index;
    final preset = ResolutionPreset.values[presetIndex.clamp(0, ResolutionPreset.values.length - 1)];
    return StreamSettings(fps: fps, colorMode: colorMode, resolutionPreset: preset);
  }
